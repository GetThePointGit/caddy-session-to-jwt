...
func (m *SessionTokenMiddleware) encrypt(bytes []byte) ([]byte, error) {

	c, err := aes.NewCipher([]byte(m.EncryptionKey))
	if err != nil {
...
func (m *SessionTokenMiddleware) decrypt(bytes []byte) ([]byte, error) {

	if len(bytes) < aes.BlockSize {
		return nil, fmt.Errorf("Invalid encrypted data")
	}

	block, err := aes.NewCipher([]byte(m.EncryptionKey))
	if err != nil {
		return nil, fmt.Errorf("Unable to create AES cipher: %v", err)
	}
